...
const SUPERUSER_ACCESS_DURATIONS_MS = {
    '24h': 24 * 60 * 60 * 1000,
    '7d': 7 * 24 * 60 * 60 * 1000,
    '30d': 30 * 24 * 60 * 60 * 1000,
};
function generateManagerPassword() {
    const plain = crypto_1.default.randomBytes(6).toString('base64url').slice(0, 8);
    const hashed = crypto_1.default.createHash('sha256').update(plain).digest('hex');
    return { plain, hashed };
}
...
router.get('/:slug/config', async (req, res) => {
    try {
        ...
        const formConfig = team.formConfig
            ? { ...DEFAULT_FORM_CONFIG, ...team.formConfig }
            : DEFAULT_FORM_CONFIG;
        ...
        const dropdownOptions = team.dropdownOptions
            ? { ...baseDropdownOptions, ...team.dropdownOptions }
            : baseDropdownOptions;
        res.json({
            id: team.id,
            slug: team.slug,
...
router.put('/:slug/config', auth_1.requireManager, async (req, res) => {
    try {
        const slug = req.params.slug;
...
router.post('/:slug/managers', auth_1.requireManager, auth_1.requireOwner, async (req, res) => {
    try {
        const slug = req.params.slug;
        ...
        const existingOwnerCount = await prisma_1.prisma.user.count({
        ...
        const [newUser] = await prisma_1.prisma.$transaction([
        ...
        let emailWarning = '';
        try {
            await (0, emailService_1.sendManagerWelcomeEmail)(normalizedEmail, plainPassword, slug, team.name, {
                logoUrl: team.logoUrl,
                primaryColor: team.primaryColor,
...
router.get('/:slug/managers', auth_1.requireManager, auth_1.requireOwner, async (req, res) => {
    try {
        const slug = req.params.slug;
        const team = await prisma_1.prisma.team.findUnique({ where: { slug } });
        if (!team) {
...
router.delete('/:slug/managers/:userId', auth_1.requireManager, auth_1.requireOwner, async (req, res) => {
    try {
        const slug = req.params.slug;
        ...
        await prisma_1.prisma.$transaction([
            prisma_1.prisma.user.delete({ where: { id: target.id } }),
            prisma_1.prisma.team.update({
                where: { id: team.id },
                data: {
                    managerEmails: team.managerEmails.filter((e) => e.toLowerCase() !== target.email.toLowerCase()),
                },
            }),
        ]);
...
router.post('/:slug/managers/:userId/resend-access', auth_1.requireManager, auth_1.requireOwner, async (req, res) => {
    try {
        const slug = req.params.slug;
        const userId = req.params.userId;
        const team = await prisma_1.prisma.team.findUnique({ where: { slug } });
        ...
        let emailWarning = '';
        try {
            await (0, emailService_1.sendManagerWelcomeEmail)(target.email, plainPassword, slug, team.name, {
                logoUrl: team.logoUrl,
                primaryColor: team.primaryColor,
                emailFromName: team.emailFromName,
            });
...
router.post('/:slug/drivers', auth_1.requireManager, async (req, res) => {
    try {
        const slug = req.params.slug;
        const { email, firstName, lastName } = req.body;
        if (!email || !firstName || !lastName) {
...
router.get('/:slug/drivers', auth_1.requireManager, async (req, res) => {
    try {
        const slug = req.params.slug;
        const team = await prisma_1.prisma.team.findUnique({ where: { slug } });
        if (!team) {
...
router.delete('/:slug/drivers/:userId', auth_1.requireManager, async (req, res) => {
    try {
        const slug = req.params.slug;
        const userId = req.params.userId;
        const team = await prisma_1.prisma.team.findUnique({ where: { slug } });
...
router.post('/:slug/drivers/:userId/resend-access', auth_1.requireManager, async (req, res) => {
    try {
        const slug = req.params.slug;
        const userId = req.params.userId;
        const team = await prisma_1.prisma.team.findUnique({ where: { slug } });
...
router.put('/:slug/superuser-access', auth_1.requireManager, auth_1.requireOwner, async (req, res) => {
    try {
        const slug = req.params.slug;
