...
const CSV_PATH = path_1.default.resolve(__dirname, '../../../Old database/Trinity_Karting_App2026-01-04_09_46_58.csv');
...
function mapClassCode(division) {
    const d = division.toLowerCase();
    if (d.includes('micro'))
        return 'Micro';
    if (d.includes('mini'))
        return 'Mini';
    if (d.includes('x30 junior') || d.includes('ka100 jr') || d.includes('oknj'))
        return 'Jr';
    if (d.includes('x30 senior') || d.includes('ka100 sr') || d.includes('okn'))
        return 'Sr';
    if (d.includes('shifter') || d.includes('kz'))
        return 'Kz';
    return 'Sr'; // Default
}
function mapMaterial(mat) {
    if (!mat)
        return undefined;
    if (mat.toLowerCase().includes('magnesium'))
        return 'Magnesium';
    if (mat.toLowerCase().includes('aluminium'))
        return 'Aluminium';
    return undefined; // Or default?
}
function mapFrontHubsMaterial(mat) {
    if (!mat)
        return undefined;
    if (mat.toLowerCase().includes('magnesium'))
        return 'Magnesium';
...
function mapHeight(h) {
    ...
    const val = h.trim();
    if (['Low', 'Medium', 'High', 'Standard'].includes(val))
        return val;
...
function mapSpindle(s) {
    // CSV: Green, Blue, Standard, Gold
    // Schema: Blue, Standard, Red, Green, Gold
    const val = s.trim();
    if (['Blue', 'Standard', 'Red', 'Green', 'Gold'].includes(val))
        return val;
    return 'Standard';
}
async function main() {
    ...
    const fileContent = fs_1.default.readFileSync(CSV_PATH, 'utf8');
